...
indice <- election_tidy_with_siglas|>
  ...
  mapa<-mapSpain::esp_get_munic(region = "CATALUÑA") |> 
...
median(as.numeric(election_data$censo))
...
ggplot(mapa_indice |> 
  ...
  scale_fill_gradient2(low = "pink",mid="purple", high = "darkblue", midpoint =454 )
...
baja_participacion <- election_tidy_with_siglas |> 
  group_by(anno) |> 
  summarise(total_participacion = sum(votos_candidaturas) / sum(censo) * 100) |> 
  filter(total_participacion < 60)

# Comparar los votos de los partidos en estas elecciones con baja participación
partido_baja_participacion <- election_tidy_with_siglas |> 
  filter(anno %in% baja_participacion$anno) |> 
  group_by(anno, Partido) |> 
  summarise(total_votos = sum(votos, na.rm = TRUE), .groups = "drop") |> 
  arrange(anno, desc(total_votos))
```
